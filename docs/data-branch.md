...
`Get Hot News` 每次读取 `data` 分支中的数据库，抓取并合并最新内容，生成增量推荐，然后提交数据库快照。代码和配置使用代码分支；`data` 是独立历史的数据分支，每次有变化时产生一个普通提交。
...
分析或通知失败后，工作流仍会尝试保存已经成功写入的数据库，且保存步骤位于网页部署之前。保存失败时任务报错，并上传保留 7 天的数据库 artifact，供恢复数据使用。推送成功到数据库提交之间的强制终止，仍可能导致下一次重试；外部通知与 Git 提交不构成同一个事务。

## 每 30 天清理

首次运行先清理过期数据，此后根据 `state.db` 记录的时间，每隔 30 天清理一次。每日数据库、HTML/TXT 快照保留最近 30 个自然日（含当天）；推荐记录和待发送内容使用 30 天滚动窗口。去重记录一旦超过窗口即不再参与过滤，无需等待物理清理。因此两次清理之间，磁盘可能暂时保存超过 30 天的文件。
...
在本地读取数据分支之前，先停止使用本地数据库的程序。以下命令会用数据分支的快照替换本地数据库集合，建议在新检出目录执行：
