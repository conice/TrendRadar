...
**多存储后端支持**：
- **数据分支持久化**：本仓库的 GitHub Actions 默认读写 `data` 分支，支持跨天去重和失败重试，详见 [数据分支说明](docs/data-branch.md)
- **远程云存储**：支持 S3 兼容协议（R2/OSS/COS 等），可按部署方式另行选择
...
### **多端部署**
- **GitHub Actions**：定时自动爬取 + `data` 分支持久化（需签到续期）
...
* **特点**：无服务器，数据库自动保存在 **`data` 分支**
* **适用**：没有服务器的用户，利用 GitHub 免费资源
* **注意**：工作流需要仓库内容写入权限，且需定期签到续期；详见 [数据分支与跨天去重](docs/data-branch.md)
...
### 5️⃣ 第五步：远程云存储 & 签到配置
   ...
   **关于远程云存储配置（请根据部署方式选择）：**
   ...
   - **GitHub Actions 用户**：
     - **默认方式**：每次从 `data` 分支恢复 SQLite，完成抓取和推荐后提交新快照，无需配置 S3。
     - **保留策略**：跨天去重窗口为 30 天，每 30 天清理过期数据；首次运行也会清理。参见 [数据分支说明](docs/data-branch.md)。
...
!价格
```
**作用：** 包含过滤词的新闻只会从**当前词组**排除；如果仍命中其他词组，仍可进入其他词组
...
[GLOBAL_FILTER]
/^(?:震惊|惊呆|吓傻)[！!:：]/ => 标题党
/(?:加|添加).{0,8}(?:微信|客服).{0,12}(?:领取|返现|进群)/ => 私域导流
...
**使用场景：**
- 过滤有明确句式的标题党、营销和私域导流话术
- 使用“行为词 + 场景词”组合，避免单个主题词造成误删
- 主题偏好优先通过增删关注词控制，不建议维护穷举式黑名单
...
本仓库默认使用本地 SQLite，并在 GitHub Actions 中通过数据分支持久化：

| 你的运行环境 | 数据存在哪 | 说明 |
|-------------|-----------|------|
| **Docker / 本地运行** | **本地硬盘** | 存在项目目录下的 `output/` 文件夹里，随时可以查看。 |
| **GitHub Actions** | **`data` 分支** | 运行前恢复数据库，运行后保存快照；跨天状态保存在 `output/state.db`。 |

详细流程、失败恢复和本地读取方法见 [数据分支与跨天去重](docs/data-branch.md)。

#### 怎么配置云存储？（可选部署方式）

也可以另行使用 Cloudflare R2 等 S3 兼容存储。切换到这种方式时，需要停用 `cross_day_dedup`，并调整当前工作流中强制使用本地数据库的配置和分支同步步骤。
...
默认保留最近 30 天的数据，首次运行清理过期数据，此后每隔 30 天清理一次。两次清理之间，磁盘上可能暂时保留更早的文件。Git 提交历史仍保留旧快照，不随文件清理而自动缩小。
...
storage:
  local:
    retention_days: 30         # 每日数据库和跨天状态保留天数（0 表示永久）
